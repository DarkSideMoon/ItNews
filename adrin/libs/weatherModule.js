...
let wunderground = new Wunderground('3d8696e7901cc512');
...
function MyWeather(city) {
    ...
    */
    ...
    that.getWeatherConditions = (callback) => {
        wunderground.conditions().request(this.city, function(err, data) { 
            if (err) {
                console.log("An error has occured. Abort!");
                callback(err);
            }
            var jsonData = JSON.parse(JSON.stringify(data));
            that.parseData(jsonData);
